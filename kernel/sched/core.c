 ...
 */
unsigned int sysctl_sched_nr_migrate = NR_CPUS * 2;
...
extern int update_irq_load_avg(struct rq *rq, int running);
#endif
...
#if SCHED_FEAT_TTWU_QUEUE
static __always_inline
bool ttwu_queue_cond(int cpu, int wake_flags)
{
	/*
...
static __always_inline
void sched_submit_work(struct task_struct *tsk)
{
